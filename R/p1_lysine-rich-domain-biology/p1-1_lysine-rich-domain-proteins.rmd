...
## Identify sub-cellular compartment with enrichment score
extractCompartment <- function(gene.set, class.name){
  ...
  # Convert enrichment scores into a data table 
  comp.out.dt <- data.table(comp.out$enrichment)
  
  #Adding additional columns - number of unmapped genes, mapped genes and class_name (gene category) 
  comp.out.dt[, `:=`(
    Genes = NA,
  ...
  # Final data table containing enrichment scores of subcellular compartment, 
  # number of unmapped genes, mapped genes and class_name (gene category) 
  return(comp.out.dt)
}

# Add column containing values from 0.0 - 0.9 only
