...
process_stoichiometry <- function(MQ_standard_data,
  ...
  for (i in seq_len(nrow(MQ_standard_data))) {
    ...
    message("Processing: ", basename(mq_evidence_data)) # Print which sample id is being processed
...
# Calculation of Stiochiometry with diagnostic ions
...
for (i in 1:nrow(MQ_DI_run_info)){ # Takes each row from the MQ_DI_run_info
  ...
  if (file.exists(mq_evidence_data)) { 
    ...
    ptm_files <- list.files(file.path(
      MQ_DI_data, MQ_DI_run_info[i, data_dir], "ptm"), 
      full.names = TRUE)
...
```{r parse_accession_function}
# Define the parse protein function 
parse_protein_accession_function_for_iterative = function(x){str_split_fixed(x, pattern = "\\|", n = 3)[, 2]}
