...
# 2.1.5 Calculation of Stoichiometry with diagnostic ion and iterative 
...
# Create file path for results
MQ_with_DI_iterative_dir <- file.path(project.dir, "results", "p2-analysis-setting", "MQ_DI_iterative")
dir.create(MQ_with_DI_iterative_dir, recursive = TRUE)
...
# Define the parse protein function 
parse_protein_accession_iterative = function(x){str_split_fixed(x, pattern = "\\|", n = 3)[, 2]}
```
...
# For each sample ID calculate the stoichiomtery
for (i in 1:nrow(MQ_DI_it_run_info)){ # Takes each row from the MQ_DI_it_run_info
  
  # Retrieves the evidence.txt file for each sample ID 
  mq_evidence_data = file.path(MQ_DI_iterative_data,
                               MQ_DI_it_run_info[i, data_dir],
                               paste0("evidence.txt"))
  
  # Pastes the sample ID that is processing 
  print(paste0("Processing: ", basename(mq_evidence_data)))
  ...
  if (file.exists(mq_evidence_data)) { 
    ...
    # Locate and fetch all PTM site files within the ptm folder
    ptm_files <- list.files(file.path(
      MQ_DI_iterative_data, MQ_DI_it_run_info[i, data_dir], "ptm"), 
      full.names = TRUE)
    ...
    stoic.dt <- calculate_stoichiometry2(
      mq_evidence_data = mq_evidence_data,
      sample_info_file = file.path( # Corresponding sample info data 
        MQ_DI_iterative_data,
        "sample_info",
        paste0(
          "MS_dataset_overview_PXD031221_",
          MQ_DI_it_run_info[i, data],
          ".csv"
        )
      ),
      ref_protein_dt = ref_protein_data,
      ptm_files = ptm_files,
      ptm_names = ptm_names,
      output_prefix = file.path(MQ_with_DI_iterative_dir,  # Define output prefix for results
                                paste0("DI_", MQ_DI_it_run_info[i, data_dir])),
      ptm_mapping_file = ptm_mapping_file,
      K_only = FALSE,
      selected_type = c("MULTI-MSMS"),
      parse_protein_accession_function = parse_protein_accession_iterative
    )
     # If evidence.txt does not exist, skip the function and print below message
  } else {
    print(paste0("File does not exist: ", mq_evidence_data))
...
# 2.1.6 Calculation of Stoichiometry with diagnostic ion, iterative and additional PTMs 

```{r data_directory_w_di_it_aPTMs}
# Load data into environment
MQ_DI_it_aPTM_data <- file.path(
  project.dir,
  "data/MQ_with_additional_ptms_output/pnas2022" 
)

# Load sample run info data 
MQ_DI_it_aPTM_run_info <- read_excel(
  file.path(project.dir, "data/analysis_setting/PXD031221_sample_matrix.xlsx"),
  sheet = "MQ_with_DI_iterative_addPTMs" 
) %>% data.table

# Add a new column 'sample_id' that assigns a unique sequential number to each 
#row (1:N), where N is the total number of rows in the data.table
MQ_DI_it_aPTM_run_info[, sample_id := 1:.N]

# Create file path for results
MQ_with_DI_it_aPTMs_dir <- file.path(project.dir, "results", "p2-analysis-setting", "MQ_DI_iterative_addPTMs")
dir.create(MQ_with_DI_it_aPTMs_dir, recursive = TRUE)
...
# For each sample ID calculate the stiochiomtery
for (i in 1:nrow(MQ_DI_it_aPTM_run_info)){ # Takes each row from the MQ_DI_it_aPTM_run_info
  
  # Retrieves the evidence.txt file for each sample ID 
  mq_evidence_data = file.path(MQ_DI_it_aPTM_data,
                               MQ_DI_it_aPTM_run_info[i, data_dir],
                               paste0("evidence.txt"))
  # Pastes the sample ID that is processing 
  print(paste0("Processing: ", basename(mq_evidence_data)))
  
   # Checks whether the sample ID has the corresponsing evidence.txt file. If yes, then proceed
  if (file.exists(mq_evidence_data)) {
    
    # Locate and fetch all PTM site files within the ptm folder
    ptm_files <- list.files(file.path(
      MQ_DI_it_aPTM_data, MQ_DI_it_aPTM_run_info[i, data_dir], "ptm"), 
      full.names = TRUE)
    ...
    stoic.dt <- calculate_stoichiometry2(
      ...
      ptm_files = ptm_files,
      ptm_names = ptm_names,
      output_prefix = file.path(MQ_with_DI_it_aPTMs_dir, paste0("DI_", MQ_DI_it_aPTM_run_info[i, data_dir])),
      ptm_mapping_file = ptm_mapping_file,
      K_only = FALSE,
      selected_type = c("MULTI-MSMS"),
      parse_protein_accession_function = parse_protein_accession_function_for_iterative
    )
...
# 2.1.6 Calculation of Stoichiometry with diagnostic ion and additional PTMs 

```{r data_directory_w_di_it_aPTMs}
# Load data into environment
MQ_DI_aPTM_data <- file.path(
  project.dir,
  "data/MQ_with_additional_ptms_output/pnas2022" 
)
...
# Create file path for results
MQ_with_DI_aPTMs_dir <- file.path(project.dir, "results", "p2-analysis-setting", "MQ_DI_addPTMs")
dir.create(MQ_with_DI_aPTMs_dir, recursive = TRUE)
...
# For each sample ID calculate the stiochiomtery
for (i in 1:nrow(MQ_DI_aPTM_run_info)){ # Takes each row from the MQ_DI_aPTM_run_info
  
  # Retrieves the evidence.txt file for each sample ID 
  mq_evidence_data = file.path(MQ_DI_aPTM_data,
                               MQ_DI_aPTM_run_info[i, data_dir],
                               paste0("evidence.txt"))
  # Pastes the sample ID that is processing 
  print(paste0("Processing: ", basename(mq_evidence_data)))
  ...
  if (file.exists(mq_evidence_data)) {
    ...
    # Locate and fetch all PTM site files within the ptm folder
    ptm_files <- list.files(file.path(
      MQ_DI_aPTM_data, MQ_DI_aPTM_run_info[i, data_dir], "ptm"), 
      full.names = TRUE)
    ...
    stoic.dt <- calculate_stoichiometry2(
      ...
      ptm_files = ptm_files,
      ptm_names = ptm_names,
      output_prefix = file.path(MQ_with_DI_aPTMs_dir, paste0("DI_", MQ_DI_aPTM_run_info[i, data_dir])),
      ptm_mapping_file = ptm_mapping_file,
      K_only = FALSE,
      selected_type = c("MULTI-MSMS"),
...
# # 2.1.7 Comparison - PNAS2022 versus new workflow for PSM coverage and stoichiometry
...
```{r comparison_PNAS2022_new_PSM_Stoic}
...
# MQ_DI - BRD4 WT
plot_ptm_stoichiometry(
  MQ_DI_stoic_dt[sample_name == "JQ1_HeLaWT_derivatised"], accession = "O60885", plot_range = c(531, 581), all.protein.bs, sample_colors = NA)

# MQ_DI - BRD4 JMJD6KO
plot_ptm_stoichiometry(
  MQ_DI_stoic_dt[sample_name == "JQ1_HeLaJMJD6KO_derivatised"], accession = "O60885", plot_range = c(531, 581), all.protein.bs, sample_colors = NA)

# MQ_DI PSM 
plot_psm_coverage(
  MQ_DI_stoic_dt[sample_name == "HeLaWT_JQ1"], accession = "O60885", plot_range = c(531, 581), all.protein.bs, sample_colors = NA)
