...
for (i in 1:nrow(MQ_DI_it_run_info)){ # Takes each row from the MQ_DI_it_run_info
  ...
  if (file.exists(mq_evidence_data)) { # Checks whether the sample ID has the corresponsing evidence.txt file. If yes, then proceed
    ...
    stoic.dt <- calculate_stoichiometry2(
      ...
      ptm_files = ptm_files,
      ptm_names = ptm_names,
      output_prefix = file.path(MQ_with_DI_iterative_dir, paste0("DI_", MQ_DI_it_run_info[i, data_dir])),
      ptm_mapping_file = ptm_mapping_file,
      K_only = FALSE,
      selected_type = c("MULTI-MSMS"),
...
for (i in 1:nrow(MQ_DI_it_aPTM_run_info)){
