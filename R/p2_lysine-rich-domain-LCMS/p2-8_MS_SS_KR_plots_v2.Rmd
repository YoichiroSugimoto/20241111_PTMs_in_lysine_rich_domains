...
#--------------------------------------------------------------
 ...
 pnas_DI_stoic_dt <- read_stoic_data(
...
pnas_DI_BRD_dt <-  pnas_DI_stoic_dt[
  gene_name %in% c("BRD2", "BRD3", "BRD4")
]
...
DI_sites <- rbind(
  ...
  pnas_DI_BRD_dt[is_diagnostic_peak == TRUE, .(protein_accession, aa_pos)]
)
...
```{r PNAS_DI_data_excel}

#--------------------------------------------------------------
# Load PNAS stoichiometry data analysed by MaxQuant (dataset A)
#--------------------------------------------------------------

 pnas_DI_dtA_stoic_dt <- read_stoic_data(
  prefix = "DI_noH2O_data-A_trp_m7_v7_def_",
  pre_prefix = "",
  post_fix = "_DI",
  dir_path = file.path(results.dir, "p2-analysis-setting", "MQ_DI_noH2O"))
...
# Create a logical column for diagnostic peak. For rows where diagnostic peak is 
# present "+", logical vector is "TRUE", or else "FALSE"
pnas_DI_dtB_stoic_dt[, `:=`(
  is_diagnostic_peak = diagnostic_peak == "+" 
)]

#--------------------------------------------------------------
# Load PNAS stoichiometry data analysed by MaxQuant (dataset C)
#--------------------------------------------------------------

 pnas_DI_dtC_stoic_dt <- read_stoic_data(
  prefix = "DI_noH2O_data-A_trp_m7_v7_mCC_",
  pre_prefix = "",
  post_fix = "_DI",
  dir_path = file.path(results.dir, "p2-analysis-setting", "MQ_DI_noH2O"))
...
# Merge MQ_DI stoic and pnas2022 by 'Protein_accession' and 'Amino_acid_pos'
 MQ_DI_PNAS_stoic_K_dt <- MQ_DI_stoic_K_dt[, `:=`( 
...
# Write to Excel table
write.xlsx(MQ_DI_PNAS_stoic_K_dt, file.path(results.dir, "p2_MS_SS_KR", "MQ_DI_PNAS_stoic_K_dt.xlsx"))
...
# Subset the data to contain only Lysine
pnas_DI_dtB_stoic_dt <- pnas_DI_dtB_stoic_dt[aa == "K"]

# removing unsed data columns from data table
pnas_DI_dtB_stoic_dt <-  pnas_DI_dtB_stoic_dt[, c("sum_intensity_per_position",
                                          "sum_psm_mapped_per_position",
                                          "sum_the_number_of_peptide",
                                          "localization_prob",
                                          "score_for_localization",
...
setcolorder(
  MQ_DI_stoic_K_dt,
  append(setdiff(names(MQ_DI_stoic_K_dt), "stoichiometry"), "stoichiometry", after = 6)
)
 
# changing sample_group column position 
setcolorder(
  MQ_DI_stoic_K_dt,
  append(setdiff(names(MQ_DI_stoic_K_dt), "is_diagnostic_peak"), "is_diagnostic_peak", after = 13)
)

 # changing column names - publication ready
setnames(MQ_DI_stoic_K_dt, c("sample_name",
                              "protein_accession",
                              "gene_name",
                              "aa",
                              "aa_pos",
...
# library("openxlsx")
 ...
 # Write to csv table to identify "K" with DI 
write.xlsx(MQ_DI_stoic_K_dt, file.path(results.dir, "p2_MS_SS_KR", "MQ_DI_stoic_K.xlsx"))
