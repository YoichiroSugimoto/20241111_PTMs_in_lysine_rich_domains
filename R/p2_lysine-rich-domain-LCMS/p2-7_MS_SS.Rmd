...
# Filter MS_KR_1 data to only contain sample name = "HeLaiJMJD6_Dox_01O224h_NA", "HeLaiJMJD6_Dox_N_NA","HeLaWT_NA_N_NA"
MS_KR1_subset_dt <- MS_KR1_stoic_dt[
  # is_diagnostic_peak == TRUE &
    # sample_name %in% c("HeLaiJMJD6_Dox_01O224h_NA", "HeLaiJMJD6_Dox_N_NA","HeLaWT_NA_N_NA") & 
    gene_name %in% c("BRD2", "BRD3", "BRD4") & 
    aa == "K"]

# Filter MS_SS_stoic to BRD proteins with diagnostic ions only
MS_SS_all_dt <-  MS_SS_stoic_dt[gene_name %in% c("BRD2", "BRD3", "BRD4") 
                                & aa == "K" &
                                 !grepl("minusDox", sample_name) 
                                #& is_diagnostic_peak == TRUE
                                ]

# Select the sites to be analysed
DI_sites <- rbind(
  MS_KR1_subset_dt[is_diagnostic_peak == TRUE, .(protein_accession, aa_pos)],
  MS_SS_all_dt[is_diagnostic_peak == TRUE, .(protein_accession, aa_pos)]
)

DI_sites <- DI_sites[!duplicated(paste(protein_accession, aa_pos))]

# grepl("^8h_21pc", sample_name),"HeLaNormoxia_8h",

# Combine MS_KR1_subset_dt and MS_SS_all_dt data
MS_SS_KR_all_dt <- rbindlist(list(MS_SS_all_dt,
                         MS_KR1_subset_dt),
                         use.names = TRUE)

# Remove BRD2/3 samples reporting BRD4 stoichiomtery and vice versa
# These samples are removed as it could false positives reported by the system 
MS_SS_KR_all_dt <- MS_SS_KR_all_dt[
  !((grepl("_BRD23$", sample_name) & gene_name == "BRD4") |
       (grepl("_BRD4$", sample_name) & gene_name %in% c("BRD2", "BRD3")))
]

MS_SS_KR_all_dt <- merge(
  DI_sites,
  MS_SS_KR_all_dt,
...
c_MS_SS_KR_all_dt <- contrast_hydroxylation_by_oxygen_stat(MS_SS_KR_all_dt)

long_c_MS_SS_KR_all_dt <- melt(
  c_MS_SS_KR_all_dt,
...
long_c_MS_SS_KR_all_dt <- long_c_MS_SS_KR_all_dt[!is.na(Stoichiometry)]

long_c_MS_SS_KR_all_dt[, data_size_per_sample := .N, by = list(sample_group)]
long_c_MS_SS_KR_all_dt[, data_size_per_site := .N, by = list(protein_accession, aa_pos)]

ggplot(
  data = long_c_MS_SS_KR_all_dt[data_size_per_site == max(long_c_MS_SS_KR_all_dt[, data_size_per_site])],
  aes(
    x = induction,
    y = Stoichiometry
  )
...
wt_21pc <- long_c_MS_SS_KR_all_dt[sample_group == "WT_Inf_21pc_KR", .(protein_accession, aa_pos, Stoichiometry)]
...
xic_stoic <- merge(long_c_MS_SS_KR_all_dt, 
                   xic_MS_SS,
  ...
  oxygen_levels =
    factor(
      fcase(
        ...
        grepl("^18h_1pc", sample_name),
        "HeLaHypoxia_18h_1pc",
        grepl("^8h_4pc", sample_name),
        "HeLaHypoxia_8h_4pc",
        grepl("^8h_1pc", sample_name),
        "HeLaHypoxia_8h_1pc",
        grepl("^4h_21pc", sample_name),
        "HeLaNormoxia_4h",
        grepl("^4h_4pc", sample_name),
        "HeLaHypoxia_4h_4pc",
        grepl("^4h_1pc", sample_name),
        "HeLaHypoxia_4h_1pc",
        grepl("^HeLai", sample_name),
        "HeLaHypoxia_24h_0.1pc",
...
contrast_hydroxylation_by_oxygen_stat <- function(all_stoic_dt){
  ...
  # Add new columns into metadata
  wt_ko_dt[, `:=`(
    oxygen_stat = str_extract(oxygen_levels, "(?<=HeLa)(WT|Normoxia|Hypoxia_18h_4pc|Hypoxia_18h_1pc|Hypoxia_8h_4pc|Hypoxia_8h_1pc|Hypoxia_4h_4pc|Hypoxia_4h_1pc|Hypoxia_24h_0.1pc)"), # new column - oxygen status, extract Normoxia or Hypoxia from oxygen_levels column
    pos_id = paste0(protein_accession, "_", aa_pos), 
    sample_pos_id = paste0(oxygen_levels, "_", protein_accession, "_", aa_pos)
  )]
