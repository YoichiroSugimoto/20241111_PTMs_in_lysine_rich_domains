...
# Remove BRD2/3 samples reporting BRD4 stoichiomtery 
...
```{r Normoxia_vs_Hypoxia_in_different_O2%_ 18h_dox}
...
ggplot(
  ...
  ylab("Stoichiometry in varying O2 levels") +
  theme_classic()+
...
MS_SS_KR_hydroxy_dt[, table(gene_name)]

# Normoxia set at 0.1
# MS_SS_KR_hydroxy_N0.1_dt <- subset(MS_SS_KR_hydroxy_dt, Normoxia >= 0.1)
# MS_SS_KR_hydroxy_N0.1_dt[, table(gene_name)]
...
m_MS_SS_KR_hydroxy_dt <- melt(
  ...
  variable.name = "sample_name", value.name = "Stoichiometry_percentage"
)
...
ggplot(
  ...
  aes(
    x = sample_name,
    y = Stoichiometry_percentage
  )
) +
  geom_boxplot(outlier.shape = NA) +
  facet_grid(~ gene_name, space = "free") +
  labs(title = "Stoichiometry") +
  theme_classic()+
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
  theme(legend.position="right") +
  coord_cartesian(ylim = c(0, 1.2))
...
```{r boxplots_stoic_diff_percent}

# Normoxia set to 0.1
# Calculating oxygen sensitivity from normoxia percentage 
MS_hydroxy_N0.1_dt <- MS_SS_KR_hydroxy_N0.1_dt[, `:=`(
  Hypo_18h_4pc = (Normoxia - Hypoxia_18h_4pc/Normoxia)*100,
  Hypo_18h_1pc = (Normoxia - Hypoxia_18h_1pc/Normoxia)*100,
  Hypo_24h_0.1pc = (Normoxia - Hypoxia_24h_0.1pc/Normoxia)*100,
  Normoxia_percent = (Normoxia/1)*100
)]

# Data converted from wide to long format 
MS_hydroxy_N0.1_dt <- melt(MS_hydroxy_N0.1_dt, id.vars = c("protein_accession", "aa_pos", "gene_name", "Normoxia_percent"),
                measure.vars = c("Hypo_18h_4pc", "Hypo_18h_1pc", "Hypo_24h_0.1pc"), 
                variable.name = "sample_name", value.name = "Stoichiometry_percentage")


# all normoxia values considered
# Calculating oxygen sensitivity from normoxia percentage 
MS_SS_KR_hydroxy_dt <- MS_SS_KR_hydroxy_dt[, `:=`(
  Stoic_percent = (Normoxia - Hypoxia_18h_4pc/Normoxia)*100,
  Stoic_percent_1 = (Normoxia - Hypoxia_18h_1pc/Normoxia)*100,
  Stoic_percent_2 = (Normoxia - Hypoxia_24h_0.1pc/Normoxia)*100,
  Normoxia_percent = (Normoxia/1)*100
)]

# Data converted from wide to long format 
MS_hydroxy_pc_dt <- melt(MS_SS_KR_hydroxy_dt, id.vars = c("protein_accession", "aa_pos", "gene_name", "Normoxia_percent"),
                measure.vars = c("Stoic_percent", "Stoic_percent_1", "Stoic_percent_2"), 
                variable.name = "sample_name", value.name = "Stoichiometry_percentage")


# Box Plot - using just stoichiometry values
ggplot(
  data = MS_hydroxy_dt,
  aes(
    x = sample_name,
...
ggplot(
  ...
  geom_point(aes(colour = gene_name)) +
  facet_grid(~ sample_name, space = "free") +
  labs(title = "Stoichiometry") +
  theme_classic()+
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
  theme(legend.position="right") 
