...
# Calculate the percentage of stoichiometric values of normoxia and hypoxia samples
# MS_hydroxy_dt <- MS_hydroxy_dt[, `:=`(
#  Stoic_percent = (Stoichiometry/1)*100
#)]
...
# Box Plot - using stoichiometry percentage values
ggplot(
  data = MS_hydroxy_dt,
  aes(
    x = sample_name,
  ...
  geom_boxplot(outlier.shape = NA) +
  facet_grid(~ gene_name, space = "free") +
  labs(title = "Stoichiometry percentage") +
  theme_classic()+
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
  theme(legend.position="right") 
