...
# fwrite(MQ_DI_noH2O_stoic_dt[ptm != ""], file = file.path(results.dir, "p2-analysis-setting", "all_hydroxylysine.csv"))

MQ_DI_noH2O_stoic_dt <- MQ_DI_noH2O_stoic_dt[aa == "K"]
MQ_DI_noH2O_stoic_dt[, genotype := str_split_fixed(sample_name, "_", n = 3)[, 2] %>% factor(levels = c("HeLaWT", "HeLaJMJD6KO"))]

# Extract the sites with diagnostic ions
DI_sites <- MQ_DI_noH2O_stoic_dt[diagnostic_peak == "+"][!duplicated(paste(protein_accession, aa_pos)), .(protein_accession, aa_pos)]

MQ_DI_noH2O_stoic_dt[, DI_site :=
...
wt_vs_KO_MQ_DI_noH2O_stoic_dt <- copy(MQ_DI_noH2O_stoic_dt)

wt_vs_KO_MQ_DI_noH2O_stoic_dt <- merge(
  wt_vs_KO_MQ_DI_noH2O_stoic_dt,
  protein.feature.dt,
  by = c("protein_accession", "aa_pos")
)

# Only analyse the sites with a coverage with 2 PSMs in both JMJD6 WT and KO data
wt_data_coverage <- wt_vs_KO_MQ_DI_noH2O_stoic_dt[genotype == "HeLaWT"][
  , list(total_wt_sum_psm_mapped = sum(sum_psm_mapped)), by = list(protein_accession, aa_pos)]
KO_data_coverage <- wt_vs_KO_MQ_DI_noH2O_stoic_dt[genotype == "HeLaJMJD6KO"][
...
wt_vs_KO_MQ_DI_noH2O_stoic_dt <- wt_vs_KO_MQ_DI_noH2O_stoic_dt[
...
wt_vs_KO_MQ_DI_noH2O_stoic_dt[!duplicated(paste(genotype, protein_accession, aa_pos))][, .N, by = genotype]

koh_wt_vs_KO_MQ_DI_noH2O_stoic_dt <- wt_vs_KO_MQ_DI_noH2O_stoic_dt[ptm == "[Oxidation (K)]"]
koh_wt_vs_KO_MQ_DI_noH2O_stoic_dt <- koh_wt_vs_KO_MQ_DI_noH2O_stoic_dt[order(-DI_site)][!duplicated(paste(genotype, protein_accession, aa_pos))]

g1 <- ggplot(
  data = koh_wt_vs_KO_MQ_DI_noH2O_stoic_dt[met_within_2 != "edge"],
  aes(
    x = genotype,
    fill = met_within_2
  )
...
g2 <- ggplot(
  data = koh_wt_vs_KO_MQ_DI_noH2O_stoic_dt[met_within_2 != "edge" & DI_site == TRUE],
  aes(
    x = genotype,
    fill = met_within_2
  )
...
koh_wt_vs_KO_MQ_DI_noH2O_stoic_dt[met_within_2 != "edge" & DI_site == TRUE][
...
koh_per_site <- dcast(
  koh_wt_vs_KO_MQ_DI_noH2O_stoic_dt,
...
merge(
  koh_wt_vs_KO_MQ_DI_noH2O_stoic_dt[!duplicated(paste(protein_accession, gene_name)), .(protein_accession, gene_name)],
...
vennlist <-  list(
  WT_DI_site = koh_wt_vs_KO_MQ_DI_noH2O_stoic_dt[genotype == "HeLaWT", paste(protein_accession, aa_pos)],
  KO_DI_site = koh_wt_vs_KO_MQ_DI_noH2O_stoic_dt[genotype == "HeLaJMJD6KO", paste(protein_accession, aa_pos)]
)
...
vennlist <-  list(
  WT_DI_site = koh_wt_vs_KO_MQ_DI_noH2O_stoic_dt[genotype == "HeLaWT" & DI_site == TRUE, paste(protein_accession, aa_pos)],
  KO_DI_site = koh_wt_vs_KO_MQ_DI_noH2O_stoic_dt[genotype == "HeLaJMJD6KO" & DI_site == TRUE, paste(protein_accession, aa_pos)]
)
...
```{r pnas2022_comparison_MQ_DI_noH2Oloss}
...
MQ_DI_noH2O_stoic_dt[, `:=`( 
  curated_oxK_site = 
    paste0(protein_accession, "_", aa_pos) %in%
    pnas2022.stoic.dt[curated_oxK_site == TRUE, paste0(protein_accession, "_", aa_pos)] 
)]

MQ_DI_d.hydroxyK_dt <- MQ_DI_noH2O_stoic_dt[ptm == "[Oxidation (K)]" & genotype == "HeLaWT"][order(genotype, DI_site)][!duplicated(paste(protein_accession, aa_pos))]
