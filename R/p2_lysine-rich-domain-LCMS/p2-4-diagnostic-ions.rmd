...
# Create a new table with two groups
feature_data_plot <- rbind(
  dt_base[, .(hydroxylation_site_class, met_within_2, group = "All")],
  dt_base[is_diagnostic_peak == TRUE, .(hydroxylation_site_class, met_within_2, group = "Diagnostic peak")] 
)

# Plot - new 
ggplot(dt_plot, aes(x = hydroxylation_site_class, fill = met_within_2)) +
  geom_bar() +
  facet_wrap(~group) +  
...
plot(venn_size_based,
     fills = list(fill = c("skyblue", "lightgreen"), alpha = 0.5),
     labels = TRUE,
     quantities = TRUE,
