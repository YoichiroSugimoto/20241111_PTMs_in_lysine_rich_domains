...
contrast_hydroxylation_by_genotype <- function(all_stoic_dt){
  ...
  d.hydroxyK_dt <- dcast(
    hydroxyK_dt,
    protein_accession + gene_name + aa_pos ~ genotype, # values in genotype become seperate columns
...
# Identify the number of hydroxylated sites in MQ_DI_it data 

MQ_DI_ <- pnas2022.stoic.dt[, .(protein_accession, aa_pos, curated_oxK_site)] %>%
  {.[order(curated_oxK_site, decreasing = TRUE)][!duplicated(paste(protein_accession, aa_pos))]}


MQ_DI_it_d.hydroxyK_dt[, .(protein_accession, aa_pos, )]
