...
In Cockman et al. (PNAS, 2022), lysine hydroxylation sites were identified by combining the following  pieces of information: (1) hydroxylysine identified through database search software (PEAKS); (2) manual inspection of MSMS spectra; and (3) the absence of hydroxylation in JMJD6 knockout cells. Here, we analysed the relationship between all the hydroxylysines identified by the software and those that satisfied conditions 2 and 3. 
...
```{r hydroxylation_sites_DI_JMJD6KO_noH20loss}
...
```{r comparison_nonDI_and_DI_noH20loss_data, fig.width = 5, fig.height = 5}
...
```{r methionine_cont_MQ_DI_noH20loss}
...
```{r comparison_PNAS2022_vs_MQ_all_vs_MQ_DI_noH20loss}
