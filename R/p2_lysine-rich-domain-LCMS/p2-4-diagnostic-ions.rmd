...
# Number of hydroxylation sites identified in PNAS2022 and new workflow

```{r Venn_diagram_PNAS2022_MQ_DI_MQ_DI_it}

##MQ_DI
# Count the total number of hydroxylations in MQ_DI
MQ_DI_stoic_dt[diagnostic_peak == "+"][aa == "K"][ptm == "[Oxidation (K)]"][order(protein_accession, aa_pos, -diagnostic_peak)][!duplicated(paste(protein_accession, genotype))][, .N]

# Count the number of hydroxylations in MQ_DI by genotype 
MQ_DI_stoic_dt[diagnostic_peak == "+"][aa == "K"][ptm == "[Oxidation (K)]"][order(protein_accession, aa_pos, -diagnostic_peak)][!duplicated(paste(protein_accession, genotype))][, .N, by = genotype]

# create data table with hydroxylated sites only
MQ_DI_total_count <- MQ_DI_stoic_dt[diagnostic_peak == "+"][aa == "K"][ptm == "[Oxidation (K)]"][order(protein_accession, aa_pos, -diagnostic_peak)][!duplicated(paste(protein_accession, genotype))]
...
##MQ_DI_it
# Count the total number of hydroxylations in  MQ_DI_it
MQ_DI_it_stoic_dt[diagnostic_peak == "+"][aa == "K"][ptm == "[Oxidation (K)]"][order(protein_accession, aa_pos, -diagnostic_peak)][!duplicated(paste(protein_accession, genotype))][, .N]

# Count the number of hydroxylations in  MQ_DI_it by genotype
MQ_DI_it_stoic_dt[diagnostic_peak == "+"][aa == "K"][ptm == "[Oxidation (K)]"][order(protein_accession, aa_pos, -diagnostic_peak)][!duplicated(paste(protein_accession, genotype))][, .N, by = genotype]

# create data table with hydroxylated sites only
MQ_DI_it_total_count <- MQ_DI_it_stoic_dt[diagnostic_peak == "+"][aa == "K"][ptm == "[Oxidation (K)]"][order(protein_accession, aa_pos, -diagnostic_peak)][!duplicated(paste(protein_accession, genotype))]

# Add new column to data table
MQ_DI_it_total_count[, `:=`(
  accession_position = paste0(protein_accession, "_", aa_pos)
)
]
...
## PNAS2022
# Create data table with hydroxylated sites only
# Note: this data set already contains the accession_protien column 
...
# Plot - Venn Diagram of comparing hydroxyöated sites between PNAS2022, MQ_DI and MQ_DI_it
...
# Extract the protein accession vectors 
vennlist <-  list(
  PNAS2022_sites = as.vector(PNAS2022_total_count[, accession_position]),
  MQ_DI_sites = as.vector(MQ_DI_total_count[, accession_position]),
  MQ_DI_it_sites = as.vector(MQ_DI_it_total_count[, accession_position])
)
